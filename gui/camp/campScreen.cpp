...
bool CampScreen::OnMouseEvent(const MouseEvent& event)
{
    bool handled = false;
...
void CampScreen::BeginCamp(bool isInn, BAK::ShopStats* shopStats)
{
...
BAK::Royals CampScreen::GetInnCost()
{
    assert(mShopStats);
    return BAK::GetRoyals(BAK::Sovereigns{mShopStats->mInnCost});
}

void CampScreen::ShowInnDialog(bool haveSlept)
{
...
bool CampScreen::AnyCharacterCanHeal()
{
    bool canHeal = false;
    mGameState.GetParty().ForEachActiveCharacter(
        [&](auto& character){
...
void CampScreen::HandleDotClicked(unsigned i)
{
    StartCamping(i);
}

unsigned CampScreen::GetHour()
{
    return mGameState.GetWorldTime().GetTime().GetHour();
}

void CampScreen::StartCamping(std::optional<unsigned> hourTil)
{
    mTargetHour = hourTil;
...
void CampScreen::FinishedTicking()
{
    if (mTimeElapser)
    {
        mTimeElapser->Stop();
...
void CampScreen::HandleButton(unsigned button)
{
