...
bool CastScreen::OnMouseEvent(const MouseEvent& event)
{
    bool handled = false;
...
void CastScreen::BeginCast(bool inCombat)
{
    mInCombat = inCombat;
...
void CastScreen::SetActiveCharacter(BAK::ActiveCharIndex character)
{
...
void CastScreen::HandleSpellClicked(BAK::SpellIndex spellIndex)
{
...
void CastScreen::HandleSpellPower(unsigned power)
{
    CastSpell(power);
    mState = State::Idle;
...
void CastScreen::CastSpell(unsigned power)
{
...
void CastScreen::HandleSpellHighlighted(BAK::SpellIndex spellIndex, bool selected)
{
...
void CastScreen::DialogFinished(const std::optional<BAK::ChoiceIndex>& choice)
{
    Exit();
}

void CastScreen::Exit()
{
    // exit lock happens to do exactly what I want.. should probably rename it
    mGuiManager.DoFade(.8, [this]{ mGuiManager.ExitLock(); });
}

void CastScreen::HandleButton(unsigned i)
{
    if (i == sExit)
    {
        Exit();
...
void CastScreen::ChangeSymbol(unsigned newSymbol)
{
