...
void DialogDisplay::ShowWorldViewPane(bool isInWorldView)
{
    if (isInWorldView)
        AddChildBack(&mWorldViewFrame);
}

void DialogDisplay::DisplayPlayer(IDialogScene& dialogScene, unsigned act)
{
...
std::pair<glm::vec2, std::string> DialogDisplay::DisplaySnippet(
...
void DialogDisplay::ShowFlavourText(BAK::Target target)
{
...
void DialogDisplay::Clear()
{
    ClearChildren();
    mWorldViewFrame.ClearChildren();
}

void DialogDisplay::AddLabel(std::string_view text)
{
    mLabel.SetText(text);
...
std::pair<glm::vec2, std::string_view> DialogDisplay::SetText(
