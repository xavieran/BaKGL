...
std::size_t Quad::GetNumVertices() const
{
    return mVertices.size();
}

QuadStorage::QuadStorage()
:
    mOffset{0},
    mVertices{},
    mTextureCoords{},
...
QuadStorage::OffsetAndLength QuadStorage::AddObject(
    const Quad& obj)
{
...
QuadStorage::OffsetAndLength QuadStorage::GetObject(std::size_t i) const
{   
    if (i >= mObjects.size())
    {
        std::stringstream ss{};
...
std::size_t QuadStorage::size() const
{
